...
//    }
    ...
    public WebElement tryToFindElement(By by) {
        ...
        try {
            element = _driver.getWait().until(ExpectedConditions.visibilityOfElementLocated(by));
        } catch (Exception e) {
    ...
    public List<WebElement> tryToFindElements(By by) {
        ...
        try {
            element = _driver.getWait().until(ExpectedConditions.visibilityOfAllElementsLocatedBy(by));
        } catch (Exception e) {
            System.out.println("Could not find element: " + by);
        }
        return element;
    ...
    public void waitOnElementAndClick(By by) {
        try {
            _driver.getWait().until(ExpectedConditions.visibilityOf(_driver.getWebDriver().findElement(by))).click();
        } catch (Exception e) {
    ...
    public void navigateToLinkInNewTab(String link) {
        ...
        ArrayList<String> tabs = new ArrayList<String>(_driver.getWebDriver().getWindowHandles());
        // Switch to the newly opened tab.
        _driver.getWebDriver().switchTo().window(tabs.get(tabs.size() - 1));
        // Navigate to the job link in that newly opened tab.
        _driver.getWebDriver().get(link);
    }
    ...
    public void switchIframes(By by) {
        _driver.getWait().until(ExpectedConditions.visibilityOfElementLocated(by));
        _driver.getWebDriver().switchTo().frame(_driver.getWebDriver().findElement(by));
